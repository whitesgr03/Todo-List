...
const sidebar = (() => {
    ...
    const createProjectItem = (project) => {
        ...
        if (li.querySelector(".title").scrollWidth > 150) {
            // 不支援觸控以及鍵盤和螢幕閱讀器使用者
    ...
    const createProjectForm = (project) => {
        ...
        if (project) {
            const color = namedColors.find(
                (color) => color.hex === project.hexCode
            );

            form.elements.name.value = project.name;
            form.firstElementChild.textContent = "Edit Project";

            form.querySelector(".colorButton").lastElementChild.textContent =
                color.name;
            form.querySelector(
                ".icon"
            ).style = `--project-color:${project.hexCode}`;
        } else {
