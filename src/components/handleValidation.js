...
const handleValidation = (() => {
    const validFormData = (formProps, form) => {
        ...
        for (let field in formProps) {
            ...
            if (
                elem.dataset.skipValid &&
                elem.classList.contains("disableOutline")
            ) {
                continue;
            }
    ...
    const validTime = (e) => {
        ...
        if (inputState.valueMissing || e.target.value.length === 0) {
            // 有選過之後又刪除值時
            e.target.value = "";
            e.target.classList.add("disableOutline");
            return;
        ...
        if (!day.classList.contains("disableOutline")) {
            // 未設定日期時會不給設定
            day.focus();
            e.target.value = "";
            message.textContent = "Date and Time cannot be set to the past";
